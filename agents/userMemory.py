...
class RecAgentRetriever(BaseRetriever):
    """RecAgentRetriever is to retrieve memories from long-term memory module based on memory salience,
    ...
    def _document_get_date(self, field: str, document: Document) -> int:
        """Return the value of the date field of a document."""
        if field in document.metadata:
            if isinstance(document.metadata[field], int):
    ...
    def _get_combined_score(
        ...
        action_times_passed = current_time - self._document_get_date("last_accessed_at", document)
        score = (1.0 - self.decay_rate) ** (action_times_passed/100)
    ...
    def get_relevant_documents_(self, query: str, now=None) -> List[Document]:
        ...
        rescored_docs = [
            (doc, self._get_combined_score(doc, relevance, now))
        ...
        for doc, _ in rescored_docs:
            if retrieved_num < self.k and doc.page_content.find('[FORGET]') == -1 \
                ...
                buffered_doc.metadata["last_accessed_at"] = now
                result.append(buffered_doc)
        return result

    def add_documents_(self, documents: List[Document], **kwargs: Any) -> List[str]:
...
class SensoryMemory:
    ...
    def _score_memory_importance(self, observation: str) -> float:
        ...
        prompt = PromptTemplate.from_template(
            """
            Please give an importance score between 1 to 10 for the following observation. 
            Higher score indicates the observation is more important. More rules that should be followed are
            \n(1) The observation that includes 'enter the recommender system' is not important. 
            e.g. David Smith enters the Amazon Recommender System to explore product recommendations based on his interests and preferences.
            \n(2) The observation that recommends or mentions specific products is important.
            \n(3) More informative indicates more important.
    ...
    def dump_shortTerm_list(self):
        ...
        order_str = "You should summarize the above observation(s) into one independent sentence." \
                    ...
                    "Note that the sentence should only pay attention to the product interest and the reasons in the " \
                    "observations. The summarization should not include the profile explicitly."
        ...
        result = [text for text in result if text[0] > 0.618]
...
class ShortTermMemory:
    ...
    def add_stm_memory(self, observation: str, importance: float, op: str):
        ...
        :param importance: the importance score of observation
        ...
        embeddings_model = OllamaEmbeddings(base_url=config['url'], model=config['emb_model_name'])
...
class LongTermMemory(BaseMemory):
    ...
    #         \n(1) The observation that includes 'enter the recommender system' is not important. e.g. David Smith enters the Amazon Recommender System to explore product recommendations based on his interests and preferences.
    #         \n(2) The observation that recommends or mentions specific products is important.
    #         \n(3) More informative indicates more important.
    #         Please respond with a single integer.
    ...
    def fetch_memories(self, observation: str, stm=None):
        ...
        :return
                ...
                (Tuple(List[str], List[float], List[str])) memories_tuple: contains the short-term memories, importance and the insights.
        """
        # reflection do not enhance the short-term memories
        # retrieved_list = self.memory_retriever.invoke(observation)
        retrieved_list = self.memory_retriever.get_relevant_documents_(observation, self.now_)
        if stm is None:
    ...
    def _get_insights_on_topic(self, topic: str):
        ...
        for idx in indexes:
            ...
            self.memory_retriever.memory_stream[idx].metadata['last_accessed_at'] = self.now_

        return result_insight

    def pause_to_reflect(self, now: Optional[int] = None):
        """
    ...
    def obtain_forget_prob_list(self):
        ...
        def score_func(importance, last_accessed_time):
            ...
            """
            recency = (1.0 - self.decay_rate) ** ((self.now_ - last_accessed_time) / 100)
    ...
    def pause_to_forget(self):
        ...
        if len(prob_list) != 0:
            for idx in range(len(prob_list)):
                if (self.now_ - self.memory_retriever.memory_stream[idx].metadata['last_accessed_at']) <= 500:
                    continue
    ...
    def add_memory(self, ltm, now=None):
        ...
        :param now: Current time. (serial version)
        ...
        result = self.memory_retriever.add_documents_([document], current_time=now)
        return result
    ...
    def save_context(self, inputs: Dict[str, Any], ltm_list: list) -> None:
        ...
        if (
            ...
            self.pause_to_reflect(now=self.now_)
...
class RecAgentMemory(BaseMemory):
    ...
    def __init__(self, llm, memory_retriever, now_, verbose=False, reflection_threshold=None):
        super(RecAgentMemory, self).__init__()
        self.now_ = now_
        self.llm = llm
        self.sensoryMemory = SensoryMemory(llm)
        self.shortTermMemory = ShortTermMemory(llm)
        self.longTermMemory = LongTermMemory(llm=llm, memory_retriever=memory_retriever, now=self.now_, verbose=verbose,
    ...
    def load_memory_variables(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        ltm_memory_list, memories_tuple = (
            self.longTermMemory.fetch_memories_with_list(inputs['observation'], self.shortTermMemory))
    ...
    def _score_memory_importance(self, memory_content: str) -> float:
        ...
        prompt = PromptTemplate.from_template(
            ...
            Higher score indicates the observation is more important. More rules that should be followed are
            \n(1) The observation that includes 'enter the recommender system' is not important. 
            e.g. David Smith enters the Amazon Recommender System to explore product recommendations 
            based on his interests and preferences.
            \n(2) The observation that recommends or mentions specific products is important.
            \n(3) More informative indicates more important.
            \nObservation:{memory_content}
            \n!!! Please respond with a single integer.!!!
    ...
    def save_context_after_retrieval(self, memories_tuple):
        ...
        :param memories_tuple:  (Tuple(List[str], List[float], List[str])) memories_tuple: contains the short-term memories, importance and the insights.
        :return None
        """
        ...
        # debug
        insight_memory_list_ = []
        for memory in insight_memory_list:
            try:
                insight_memory_list_.append(memory[0])
            except:
                print(str(insight_memory_list))
                pass
        insight_memory_list = insight_memory_list_
        ...
        save_ltm_memory = [(all_memory_scores[i], self.now_, all_memories[i])
    ...
    def save_context(self, inputs: Dict[str, Any], outputs: Dict[str, str]) -> None:
        ...
        if stm_memory_list is None:
            ...
            ''' # action sequence as time flow. # 24-8-22 by zhr.'''
            self.now_ += 1
